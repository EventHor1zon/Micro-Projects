...
/******* Defines ***********/
/*  Mode config; 0= tcp connection to server, 1=host http web page 2=Both */
...
static void transmit_sample(void *args){
    ...
    while(1){
        ...
        /*
        int recvd = recv(sockfd_0, rx_buffer, sizeof(rx_buffer)-1, 0);
...
static void websocket_serve(void *args){
    ...
    while(1){
        ...
        if(!(uxQueueMessagesWaiting(qHandle)) || !CONNECTED_STATUS){
            ...
            //char data[13+sizeof(uint32_t)+1];
            //int len = snprintf(data, sizeof(data), "{\"Uptime\":\"%"PRIu32"\"}", testval);

            int16_t aX = data.Accelerometer_X;
            int16_t aY = data.Accelerometer_Y;
            int16_t aZ = data.Accelerometer_Z;
            char data[(sizeof(int16_t)*3 )+ 26];
            int len = snprintf(data, sizeof(data), "{\"aX\":\"%d\",\"aY\":\"%d\",\"aZ\":\"%d\"}", aX, aY, aZ);
...
static void ws_recv_cb(struct tcp_pcb *tcp, uint8_t data, uint16_t data_len, uint8_t mode){
    ...
    //ESP_LOGI(WS_TAG, " Data received: %s", (char *)data);
    uint8_t reply[2]={0x41, 0x41};
    websocket_write(tcp, reply, 2, WS_BIN_MODE);
